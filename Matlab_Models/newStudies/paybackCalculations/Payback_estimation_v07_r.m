...
pv_cap_nominates        = [3,5,7,10];  %linspace(1,10,10);
...
for pv_size = pv_cap_nominates
    ...
    for battery_size = battery_cap_nominates
        ...
        for index = 1:length(lastMonthDemand ) %load_energy_demand)
            ...
            if  lastmonthtimetable == 0 % priceDatabase2022.time(index,1)==0      %year_price(index)*1000 < 250   %ismember(index,last_hour_cheap_energy) 
            ...
            if index == PriceminIndex && chargePermision
                 %PriceminIndex   = find (year_price(index:index+23),)
%                 if index > 8736-16 
%                     index = 8736-16;
%                 end
