...
Inv_PriceB              = 650; %950;                 % Investment cost of battery ($/kWh).
Inv_PricePV             = 1100;
...
battery_cap_nominates   = linspace(0,15,151);                     % Different battery capacity with the step of 0.1
pv_cap_nominates        = linspace(1,20,20);
...
for pv_size = pv_cap_nominates
    ...
    for battery_size = battery_cap_nominates
        ...
        investment_battery_pv(index_pv,index_battery) = battery_size * (Inv_PriceB) + Inv_PricePV * pv_size + PB_max*200;  %1700
...
for i = pv_cap_nominates
    ...
    plot (0:0.1:15 , payback_time(j,:),'DisplayName',txt) 
