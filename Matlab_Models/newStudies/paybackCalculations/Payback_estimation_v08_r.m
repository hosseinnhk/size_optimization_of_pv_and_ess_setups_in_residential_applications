...
cost_grid               = sum(lastMonthDemand .* lastmonthprice);
...
for pv_size = pv_cap_nominates
    ...
    for battery_size = battery_cap_nominates
        ...
        for index = 1:length(lastMonthDemand ) %load_energy_demand)

            pv_realtime_value   = lastMonthGeneration(index); % pv_generated_energy(index);
            load_realtime_value = lastMonthDemand(index);     % load_energy_demand(index);
            ...
            % pv cann't handle the load demand and we will either battery or grid.
                ...
                if index > 1 && lastmonthprice(index) > 1.3 * priceMinNextDay 
                load_realtime_value            = ...
            ...
            if  lastmonthtimetable(index) == 0 % priceDatabase2022.time(index,1)==0      %year_price(index)*1000 < 250   %ismember(index,last_hour_cheap_energy) 
                [priceMinNextDay,PriceminIndex] = min(lastmonthprice(index:index+23)); % the chepeaset hour in th next 24hours
                if sum(lastMonthGeneration(index:index+23)) < sum(lastMonthDemand(index:index+23)) 
            ...
            if index == PriceminIndex && chargePermision
                ...
                %if sum(pv_generated_energy(index:index+23)) < sum(load_energy_demand(index:index+23)) 
        ...
        cost_pv_battery(index_pv,index_battery)       = sum(energy_usage_grid .* lastmonthprice);    
...
for i = pv_cap_nominates
    ...
    plot (0:0.1:30 , payback_time(j,:),'DisplayName',txt) %find(pv_cap_nominates==i)
    hold on;
    plot(best_battery_size/10,mininum_paybacktime,'*r')
end
...
for i = pv_cap_nominates 
    j = find(pv_cap_nominates==i);
    txt = ['pv size= ',num2str(i),' kwh','       paybacktime= ', num2str(find(profit(j,:) > 0, 1 , 'first')),' years'];  % > 0, 1, 'first')) num2str(profit(j,:))
