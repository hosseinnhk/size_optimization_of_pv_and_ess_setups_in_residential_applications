...
% -------------------------------------------------------------------------Ess------------------------------------------------------
ess.enR_max             = 7000; %linspace(2.5,20,8).*1000;                 % Maximum delivarable power exchange with battery. W
ess.nominates           = 5000; %linspace(1,15,15).*1000;                  % Different battery capacity   5000; 
...
for Enr_size = ess.enR_max
    ...
    for pv_size = pv.nominates
        ...
        for ess_size = ess.nominates
            ...
            ess.parallelCellNum        = ess.prlCell_nom( ess.nominates/1000  );    %   ess.nominates/1000
                          ...
                          if energy.flow(index) > 0    % energy surplus
                           ...
                           if  energy.genPred <= energy.demPred % we should charge battery for the near future
                                ...
                                if energy.flow(index) > 0
                                    log.utl.feedinEnergy(ess.enRindex, pv.index, ess.index,index) = min(energy.flow(index),ess.enrSize);
                                    supply.energyPVgrid (ess.enRindex, pv.index, ess.index,index) = min(energy.flow(index),ess.enrSize);
                                    utl.feedinBill(index) = utl.feedinTrf(index)*log.utl.feedinEnergy(ess.enRindex, pv.index, ess.index,index);
                                    energy.flow(index)    = 0;
                                ...
                                log.utl.feedinEnergy(ess.enRindex, pv.index, ess.index,index) = min(energy.flow(index),ess.enrSize);
                                supply.energyPVgrid (ess.enRindex, pv.index, ess.index,index) = min(energy.flow(index),ess.enrSize);
                                utl.feedinBill(index) = utl.feedinTrf(index)*log.utl.feedinEnergy(ess.enRindex, pv.index, ess.index,index);  %usd
                                energy.flow(index) = 0; 
                           end
...
%                                     if (demand.energyUtl(ess.enRindex, pv.index, ess.index,index) > ess.enrSize)
%                                         fprintf("need for energy router limit1\n")
%                                     end
                                ...
                                demand.energyUtl(ess.enRindex, pv.index, ess.index,index) = -energy.flow(index);
%                                 if (demand.energyUtl(ess.enRindex, pv.index, ess.index,index) > ess.enrSize)
%                                     fprintf("need for energy router limit2\n")
%                                 end                                
                                log.utl.importEnergy(ess.enRindex, pv.index, ess.index,index) = -energy.flow(index);
                                utl.smartBill(index) = utl.trf(index)*-energy.flow(index);
                                energy.flow(index)   = 0;                            
                    ...
                    case utl.offPeakPr
                         ...
                         elseif energy.flow(index) < 0 % energy shortage
                            ...
                            demand.energyUtl(ess.enRindex, pv.index, ess.index,index) = -energy.flow(index);
%                             if (demand.energyUtl(ess.enRindex, pv.index, ess.index,index) > ess.enrSize)
%                                 fprintf("need for energy router limit3\n")
%                             end                            
                            log.utl.importEnergy(ess.enRindex, pv.index, ess.index,index) = -energy.flow(index);
                            utl.smartBill(index) = utl.trf(index)*-energy.flow(index);
                            energy.flow(index)   = 0;  
                         ...
                         if ess.soc(index) < 0.6 && utl.trf(index) == utl.trfMin   % ess charge from grid
                            ...
                            if (ess.energy(index)-dummy.essEnergy) > ess.enrSize
                                fprintf("need for energy router limit4\n")
                            end                                                                            
                    ...
                    case utl.peakPr
                         ...
                         elseif energy.flow(index) < 0 % energy shortage
                            ...
                            if energy.flow(index) < 0  
                                demand.energyUtl(ess.enRindex, pv.index, ess.index,index) = -energy.flow(index);
%                                 if (demand.energyUtl(ess.enRindex, pv.index, ess.index,index) > ess.enrSize)
%                                     fprintf("need for energy router limit6\n")
%                                 end 
                                log.utl.importEnergy(ess.enRindex, pv.index, ess.index,index) = -energy.flow(index);
                                utl.smartBill(index) = utl.trf(index)*-energy.flow(index);
                                energy.flow(index)   = 0;
...
%                               end
                            ...
                            log.utl.feedinEnergy(ess.enRindex, pv.index, ess.index,index) = log.utl.feedinEnergy(ess.enRindex, pv.index, ess.index,index) + ...
                                                                          (dummy.essEnergy - ess.energy(index)); 
                            supply.energyEssgrid (ess.enRindex, pv.index, ess.index,index) = dummy.essEnergy - ess.energy(index);
                            if (supply.energyEssgrid (ess.enRindex, pv.index, ess.index,index) > ess.enrSize*1.031)
                                fprintf("need for energy router limit7\n")
                            end                             
