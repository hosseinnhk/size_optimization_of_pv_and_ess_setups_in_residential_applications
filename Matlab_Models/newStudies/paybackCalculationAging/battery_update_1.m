function [storageEnergy,StateOfHealth,unprocessedEnergy,SoC] = battery_update(index,Ess,Ess_size,status,energy)
...
%state of the health calculations:
   ...
   if strcmpi(status,'discharge')
         currentAh2 = Ess.Qnorm*(1-SoC_final); 
         currentAh1 = Ess.Qnorm*(1-SoC_init);
    elseif strcmpi(status,'charge')
        currentAh1 = Ess.Qnorm*(1-SoC_final); 
        currentAh2 = Ess.Qnorm*(1-SoC_init);
    end

    SoC_avg = (1/deltaAh) *( (-(currentAh2^2)/(2*Ess.Qnorm) + currentAh2) - ...
                             (-(currentAh1^2)/(2*Ess.Qnorm) + currentAh1) );
    SoC_dev = (3/deltaAh) * ( ( (currentAh2^3/(3 * Ess.Qnorm^2)) + currentAh2 + currentAh2*SoC_avg^2 - currentAh2^2/Ess.Qnorm - 2*SoC_avg*currentAh2 + (currentAh2^2/Ess.Qnorm)* SoC_avg )   - ...
                              ( (currentAh1^3/(3 * Ess.Qnorm^2)) + currentAh1 + currentAh1*SoC_avg^2 - currentAh1^2/Ess.Qnorm - 2*SoC_avg*currentAh1 + (currentAh1^2/Ess.Qnorm)* SoC_avg ) );
    ...
    %StateOfHealth = (1 - zita/(0.2 * Ess.Qnorm)); %Ess.SoH(index)
    StateOfHealth = -zita/(0.2 * Ess.Qnorm); %Ess.SoH(index)
