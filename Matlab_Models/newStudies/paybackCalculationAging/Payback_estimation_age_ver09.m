...
% in this version (9) system will run for 15 years with new strategy for
% battery charging and also pb max 5k 10k is applied in this version
...
pv.nominates            = (1:20) *1000;  %7000; %
pwr.Max                 = 10000;  %10000;         % Maximum of load demand during a year. W
pv.index                = 0;
...
ess.PB_max              = 20000;                % Maximum delivarable power exchange with battery. W
...
% -------------------------------------------------------------------------Simulation----------------------------------------------------------
simu.period              = 15;   % year
simu.dur                 = linspace(1,20,20);
simu.essSamples          = length(ess.nominates);
simu.pvSamples           = length(pv.nominates);


% Financial parameters              
inv.ess                 = 650;                 % cost of battery ($/kWh). based on Bloomberg data
inv.pv                  = 1100;                % cost of PV ($/kWp). 
inv.pvpriceSizedec      = 0.04;
inv.infr                = 200;                 % cost of infrastructures ($/kW).
...
for pv_size = pv.nominates
    ...
    for ess_size = ess.nominates
        ...
        ess.parallelCellNum        = ess.prlCell_nom(ess.index );    %ess.index ess.nominates/1000
        ess.seriesCellnum          = ess.srCell_nom (ess.index );   %  ess.nominates/1000
            ...
            switch utl.state
                case utl.normalPr
                      ...
                      if energy.flow(index) > 0    % energy surplus
                       ...
                       if  energy.genPred <= energy.demPred % we should charge battery for the near future
                            ...
                            battery_update_6(index,ess,'charge',energy.flow(index),soc);
                     ...
                     elseif energy.flow(index) < 0  % energy shortage
                        ...
                        if  energy.genPred >= energy.demPred   
                            ...
                            battery_update_6(index,ess,'discharge',energy.flow(index),soc);
                            demand.energyEss(pv.index, ess.index,index) = dummy.essEnergy - abs(energy.flow(index));

                            if demand.energyEss(pv.index, ess.index,index)<0
                                 fprintf("Warning2\n");
                                 demand.energyEss(pv.index, ess.index,index)
                                 energy.flow(index)
                            end
                ...
                case utl.offPeakPr
                     ...
                     if ess.soc(index) < 0.6 && utl.trf(index) == utl.trfMin   % ess charge from grid
                        ...
                        %ess.fullCharge  = min(ess.PB_max, ess.fullCharge);  %power electronic limit
                 
                        [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.soc(index),dummy.soc,ess.current(index)]= ...
                        battery_update_6(index,ess,'charge',ess.fullCharge,soc);
                ...
                case utl.peakPr
                     ...
                     if ess.soc(index) > 0.4 && utl.trf(index) == utl.trfMax   % energy export to grid at max point 
                        ...
                        [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.soc(index),dummy.soc,ess.current(index)]= ...
                        battery_update_6(index,ess,'discharge',-ess.energy(index),soc);
...
%            if (dummy.year == 10 || ess.soh(index) <= 0.6) && ess.replace==0
% 
%                ess.replace              = 1;
%                ess.cellFade             = zeros((simu.length));
%                ess.soh(index)           = 1; %ess.soh(index) * (0.99995^(index)); % calendar age factor  1-0.98/(365*24)
%                ess.soc(index)           = .9;
...
%            end
           ...
           if rem(index,8736)==0
                ...
                inv.essPv(pv.index,ess.index) = ess.cap * (inv.ess*(1-ess.priceSizedec)^(ess.cap/1000))/1000 + ...
                    (inv.pv*(1-inv.pvpriceSizedec)^(pv_size/1000)) * pv_size/1000 + inv.infr*ess.PB_max/1000;
                
                inv.maintenance(dummy.year) = inv.essPv(pv.index,ess.index) * 0.01*(1+inv.annualInflation)^dummy.year;

                if dummy.year == 10
                    inv.replace(dummy.year) = ess.cap*(1+inv.annualInflation)^dummy.year* inv.ess/(1000*(1+ess.pricedecrease)^10);
                elseif dummy.year == 15
                    inv.replace(dummy.year) = inv.infr*ess.PB_max*(1+inv.annualInflation)^dummy.year/(1000*(1+.05)^15);  
                else    
                ...
                -inv.essPv(pv.index,ess.index) - sum (inv.maintenance) - sum(inv.replace( 1: dummy.year))...
        ...
        log.ess.cycleCharge(pv.index,ess.index,:)         = soc.cycleCharge';
        log.ess.cycleDischarge(pv.index,ess.index,:)      = soc.cycleDischarge';
        log.ess.current(pv.index,ess.index,:)             = ess.current';
