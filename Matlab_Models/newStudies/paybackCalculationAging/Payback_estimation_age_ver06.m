...
pv.nominates            = 7000; %[3,5,7,10]; %[2,3,4,5,6,7,8,9,10]; 
...
ess.nominates           = 10000; %linspace(1,8,8);   %linspace(1,15,15);                     % Different battery capacity with the step of 1 Wh.
            ...
            switch utl.state
                case utl.normalPr
                     ...
                     if energy.flow(index) > 0    % energy surplus
                        ...
                        if  energy.genPred <= energy.demPred % we should charge battery for the near future
                            ...
                            battery_update_3(index,ess,ess_size,'charge',energy.flow(index),soc);
                     ...
                     elseif energy.flow(index) < 0
                        ...
                        if  energy.genPred >= energy.demPred   
                            ...
                            battery_update_3(index,ess,ess_size,'discharge',energy.flow(index),soc);
                            if energy.flow(index) < 0
                ...
                case utl.offPeakPr
                     ...
                     if ess.SoC(index) < 0.6 && utl.trf(index) == utl.trfMin   % ess charge from grid
                        ...
                        [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.SoC(index),dummy.soc]= ...
                        battery_update_3(index,ess,ess_size,'charge',ess.energy(index),soc);
                ...
                case utl.peakPr
                     ...
                     if ess.SoC(index) > 0.5 && utl.trf(index) == utl.trfMax   % energy export to grid at max point 
                        dummy.essEnergy = ess.energy(index);
                        [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.SoC(index),dummy.soc]= ...
                        battery_update_3(index,ess,ess_size,'discharge',ess.energy(index),soc);
...
% 
% 
%                if  ess.pvToess && utl.trf(index) < utl.trfMax/2        %ess.chargeBenefit < utl.feedBenefit
...
%                     ess.totalCurrent     = energy.flow(index)/ ess.packVoltage ; %Ah
%                     ess.cellCurrent      = ess.totalCurrent / ess.parallelCellNum;  %Ah
%         
%                     [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.SoC(index)]= ...
%                         battery_update_3(index,ess,ess_size,'charge',energy.flow(index)); 
% 
%                     if energy.flow(index) > 0
%                         utl.feedinBill(index) = utl.feedinTrf(index)*energy.flow(index);
%                         energy.flow(index)  = 0;
...
% 
%                     [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.SoC(index)]= ...
%                         battery_update_3(index,ess,ess_size,'discharge',energy.flow(index)); 
% 
%                     if energy.flow(index) < 0
%                         utl.smartBill(index) = utl.trf(index)*energy.flow(index);
% 
...
%                 [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.SoC(index)]= ...
%                         battery_update_3(index,ess,ess_size,'charge',ess_size*ess.SoC(index));
...
%                 dummy.fade = ess.cellFade(index);
%                 [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.SoC(index)]= ...
%                         battery_update_3(index,ess,ess_size,'discharge',ess_size*ess.SoC(index));
%                 ess.cellFade(index) = dummy.fade + ess.cellFade(index);
%             end
% 
...
%            end
           ...
           %ess.SoH(index) = ess.SoH(index) - ess.cellFade(index);
           ess.SoH(index) = 1 - sum(ess.cellFade)/(0.2* ess.Qnom);
            % ess.SoH(index) =  ess.cellFade(index);
...
%                         [storage_energy(index),SoH(index)] = battery_update_3(storage_energy(index),...
...
%                 storage_energy(index) = 0; %battery_update_3(battery_charge_capacity(index),...
