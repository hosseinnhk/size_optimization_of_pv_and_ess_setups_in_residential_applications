function [storageEnergy,ess_degration,unprocessedEnergy,socFinal,soc,cellCurrent] = battery_update_7(index,Ess,status,energy,Soc)
    
    if energy < 0
       possible_energy = -min(abs(energy),Ess.enrSize);
       
    else
       possible_energy = min(energy,Ess.enrSize); 
    end
   ...
   if strcmpi(status,'charge') 
        dummy_waste = possible_energy;
        possible_energy = Ess.energyRouterEf * possible_energy;
        dummy_waste = dummy_waste - possible_energy;
        ...
        if possible_energy >= dummy_value
            socFinal = Ess.maxsoc;
            unprocessedEnergy = energy - dummy_value - dummy_waste;
        else
            socFinal = Ess.soc(index) + possible_energy/Ess_size;
            unprocessedEnergy = energy - possible_energy - dummy_waste;
        end
