...
for pv_size = pv.nominates
    ...
    for ess_size = ess.nominates*1000
        ...
        energy.flow              = (pv.database - demand.profile)*1000; % positive surplus negative shortage.
...
%             end
            ...
            % Calculation of system state and costs
               %ess.dischargeCost(index) = utl.feedinTrf(index) * ess.roundTripEf * (ess.energy(index) - energy.flow(index));
            switch utl.state
                case utl.normalPr
                     ...
                     if energy.flow(index) > 0    % energy surplus
                        ...
                        if  energy.genPred <= energy.demPred % we should charge battery for the near future
                            ...
                            battery_update2(index,ess,ess_size,'charge',energy.flow(index));
                     ...
                     elseif energy.flow(index) < 0
                        ...
                        if  energy.genPred >= energy.demPred   
                            [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.SoC(index)]= ...
                            battery_update2(index,ess,ess_size,'discharge',energy.flow(index));
                            if energy.flow(index) < 0
...
%                         else 
 ...
 %                       end
                     ...
                     if ess.SoC(index) < 0.6 && utl.trf(index) == utl.trfMin   % ess charge from grid
                        ...
                        [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.SoC(index)]= ...
                        battery_update2(index,ess,ess_size,'charge',ess.energy(index));
...
%                         end
                     ...
                     if ess.SoC(index) > 0.5 && utl.trf(index) == utl.trfMax   % energy export to grid at max point 
                        dummy.essEnergy = ess.energy(index);
                        [ess.energy(index),ess.cellFade(index),energy.flow(index),ess.SoC(index)]= ...
                        battery_update2(index,ess,ess_size,'discharge',ess.energy(index));
