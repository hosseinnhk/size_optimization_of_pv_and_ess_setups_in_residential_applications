% Simple scheduling
% calculations for various pv and battery sizes and comparison of payback time and inv.profit during 100 years.
...
pv.nominates            = [2,3,4,5,6,7,8,9,10]; 
...
ess.nominates           = linspace(1,15,15);                     % Different battery capacity with the step of 1 kWh.
...
ess.optSize             = zeros(length(pv.nominates),1);
...
for pv_size = pv.nominates
    ...
    for ess_size = ess.nominates
        ...
        ess.energy(1)              = ess.minSoC* ess_size;
        ...
        for index = 1:length(demand.profile)
            ...
            % Calculation of system state and costs
               ...
               if 
               ess.dischargeCost = utl.feedin(index)*ess.RoundTripEf*;
            ...
            if energy.fellow(index) > 0 % energy surplus
               ...
               if ess.chargeBenefit < utl.feedBenefit
            ...
            elseif energy.fellow(index) < 0 % energy shortage
               ...
               if ess.dischargeCost < utl.importCost 
            ...
            switch machine.state 
                ...
                case state.essCharge
                    ...
                    ess.totalCurrent     = energy.fellow(index)/ ess.Voltage; %Ah
                    ess.CellCurrent      = ess.totalCurrent / ess.parallelCellNum;  %Ah
                    [ess.energy(index),ess.SoH(index),energy.fellow(index)]= ...
                        battery_update(index,ess,ess_size,'charge'); 
                    if energy.fellow(index) > 0
                        utl.feedinBill(index) = utl.feedin(index)*energy.fellow(index);
                ...
                case state.essDischarge
                    ...
                    ess.CellCurrent      = ess.totalCurrent / ess.parallelCellNum;  %Ah
                    [ess.energy(index),ess.SoH(index),energy.fellow(index)]= ...
                        battery_update(index,ess,ess_size,'discharge'); 
                    if energy.fellow(index) < 0
...
%         ess.dummyIndex                                 = find(ess.nominates == ess_size);
%         pv.index                                       = find(pv.nominates == pv_size);
%         inv.oprCost(pv.index,ess.dummyIndex)           = sum(utl.energy.* utl.trf);    
%         inv.essPv(pv.index,ess.dummyIndex)             = ess_size * (inv.ess) + inv.pv * pv_size; %+PB_max*10  1700
%         inv.payback(pv.index,ess.dummyIndex)           = ...
%             inv.essPv(pv.index,ess.dummyIndex) ./ (utl.bill - inv.oprCost(pv.index,ess.dummyIndex)); 
    end      
end
...
%         inv.profit(i,j) =  - inv.essPv(i,best_battery_size(i)) + simu.dur(j) * (cost_grid - inv.oprCost(i,best_battery_size(i)));
%     end
% end
% 
...
%     plot (0:0.1:30 , inv.payback(j,:),'DisplayName',txt) 
%     %plot (inv.payback(j,:),'DisplayName',txt)
%     hold on;
%     plot(best_battery_size/10,utl.minpaybackS,'*r','HandleVisibility','off');
% end
% 
