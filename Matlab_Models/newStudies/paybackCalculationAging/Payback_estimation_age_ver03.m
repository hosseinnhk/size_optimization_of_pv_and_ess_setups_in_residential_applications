...
utl.bill               = sum(demand.profile .* utl.trf)*1000;
...
ess.SoHt                = zeros(simu.pvSamples, simu.essSamples, simu.length(1,2));
...
for pv_size = pv.nominates
    ...
    for ess_size = ess.nominates*1000
        ...
        %ess.Qnorm                  = ess_size/ess.Voltage; 
           ...
           switch machine.state 
                ...
                case state.essCharge
                    ...
                    [ess.energy(index),ess.cellFade(index),energy.fellow(index),ess.SoC(index)]= ...
                        battery_update(index,ess,ess_size,'charge',energy.fellow(index)); 
                ...
                case state.essDischarge
                    ...
                    [ess.energy(index),ess.cellFade(index),energy.fellow(index),ess.SoC(index)]= ...
                        battery_update(index,ess,ess_size,'discharge',energy.fellow(index)); 
...
%                             ess_size,ess_size,'charge',PB_max,SOC_min,ess.maxSoC,Qnorm,EnergyRouterEf);
...
%         end
        ...
        inv.payback(pv.index,ess.index) = ... 
             inv.essPv(pv.index,ess.index) ./ (utl.bill - inv.oprCost(pv.index,ess.index)); 
    end      
end
