...
% -------------------------------------------------------------------------Ess------------------------------------------------------
ess.enR_max             = 5000; %linspace(2.5,20,8).*1000;                 % Maximum delivarable power exchange with battery. W
                ...
                switch utl.state
                          ...
                          if energy.flow(index) > 0    % energy surplus
                           ...
                           if  energy.genPred <= energy.demPred % we should charge battery for the near future
                                ...
                                if energy.flow(index) > 0
                                    ...
                                    supply.energyPVgrid (ess.enRindex, pv.index, ess.index,index) = energy.flow(index);
                    ...
                    case utl.offPeakPr
                         ...
                         if ess.soc(index) < 0.6 && utl.trf(index) == utl.trfMin   % ess charge from grid
                            ...
                            supply.energygridEss (ess.enRindex, pv.index, ess.index,index) = ess.fullCharge;
                            log.utl.importEnergy(ess.enRindex, pv.index, ess.index,index) = ...
                            log.utl.importEnergy(ess.enRindex, pv.index, ess.index,index) + (ess.fullCharge );   %ess.energy(index) - dummy.essEnergy
                                                                           
                            if ess.energy(index) < dummy.essEnergy
                               fprintf("Warning4/n");  
                            end
    
                            utl.smartBill(index) = utl.smartBill(index) + utl.trf(index)*(ess.fullCharge); %ess.energy(index) - dummy.essEnergy
