...
dummy.range = 0:23;
dummy.count = 1;
dummy.lastEl = size(elecPriceData1031.time(:,1));

%replacing time am pm format by dec numbers. 
% while dummy.count < dummy.lastEl(1,1)
%     for i= dummy.range 
%         elecPriceData1031.time(dummy.count + i) = i;        
%     end
%     dummy.count = dummy.count + 24 ;
% end

% selecting 2022 data
priceDatabase2022 = elecPriceData1031(2:end,:);
% priceDatabase2022 = [priceDatabase2022 ; priceDatabase2022(5833-24:6552,:)];    % oct 31 *1.1025 
priceDatabase2022 = [priceDatabase2022 ; priceDatabase2022(6552:7272,:)];       % Nov 30 *1.1576
priceDatabase2022 = [priceDatabase2022 ; priceDatabase2022(6552:7271,:)];       % Dec 31 *1.215   MWh
...
% adding 2 percent inlflation for next 2 monthes of 2022
%utl.trf(6553:7296,1) = utl.trf(6553:7296,1)* 1.02; 
utl.trf(7298:8016,1) = utl.trf(7298:8016,1)* 1.02; 
...
ess.enR_max             = linspace(5,20,4).*1000; %;20000;                % Maximum delivarable power exchange with battery. W
ess.enRindex            = 0; 
...
inv.payback             = zeros((simu.hardwareSamples));               % payback time calculation
...
for Enr_size = ess.enR_max
    ...
    for pv_size = pv.nominates
        ...
        for ess_size = ess.nominates
            ...
            ess.seriesCellnum          = ess.srCell_nom ( ess.index );   %   ess.index
            ...
            for index = 1:length(demand.profile)
                ...
                switch utl.state
                    case utl.normalPr
                         ...
                         elseif energy.flow(index) < 0  % energy shortage
                            ...
                            if  energy.genPred >= energy.demPred   
                                ...
                                demand.energyEss(ess.enRindex, pv.index, ess.index,index) = dummy.essEnergy - abs(energy.flow(index));
                    ...
                    case utl.offPeakPr
                         ...
                         if ess.soc(index) < 0.6 && utl.trf(index) == utl.trfMin   % ess charge from grid
                            ...
                            log.utl.importEnergy(ess.enRindex, pv.index, ess.index,index) = log.utl.importEnergy(ess.enRindex, pv.index, ess.index,index) + ...
               ...
               if (dummy.year == 10 || ess.soh(index) <= 0.65) && ess.replace==0
                   ...
                   ess.BESchange(ess.enRindex, pv.index, ess.index) = index;
                   ess.replace              = 1;
                   ess.cellFade             = zeros((simu.length));
               ...
               if rem(index,8736)==0
                    ...
                    inv.essPv(ess.enRindex, pv.index,ess.index) = ess.cap * (inv.ess*(1-ess.priceSizedec)^(ess.cap/1000))/1000 + ...
            ...
            utl.netSmartBill(ess.enRindex, pv.index,ess.index)              = sum(abs(utl.smartBill)-utl.feedinBill);
