...
Counter=0;
for CapB=5               % Runin the system for different battery capacity with the step of 0.1
...
%          
        ...
        My_PL(My_PL<0)=0;
        Cost1=Cost1+sum(Price'.*[PL(1:T1-1);My_PL(T1:T2);PL(T2+1:24)]);
        Cost2=Cost2+sum(Price.*My_LD);
        Cost3=Cost3+sum(Price'.*PL);
        
    end
    COST1=[COST1;Cost1];
    COST2=[COST2;Cost2-Memory*Price(end)];
    COST3=[COST3;Cost3];
    INV_Cost=[INV_Cost;((Inv_PriceB*CapB)+5000)];
end
%figure;plot(0:0.1:15,INV_Cost./(COST1-COST2))
%figure;plot(0:0.1:15,INV_Cost./(COST3-COST2))
[a b]=min(INV_Cost./(COST1-COST2));
PBplot=squeeze(PB(b,:,:))';
figure;plot(PBplot(:))
hold on
